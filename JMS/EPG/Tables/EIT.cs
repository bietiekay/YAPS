...
namespace JMS.DVB.EPG.Tables
{
	...
	/// <item><term>2/0</term><description>Current/Next Indicator, which is stored in <see cref="IsCurrent"/></description></item>
	/// <item><term>3/7-0</term><description><see cref="SectionNumber"/></description></item>
	/// <item><term>4/7-0</term><description><see cref="LastSectionNumber"/></description></item>
	...
	public class EIT: Table
	{
		...
		public EIT(Section section) : base(section)
		{
			...
			// Construct
			TransportStreamIdentifier = Tools.MergeBytesToWord(section[6], section[5]);
			OriginalNetworkIdentifier = Tools.MergeBytesToWord(section[8], section[7]);
			ServiceIdentifier = Tools.MergeBytesToWord(section[1], section[0]);
			...
			// Convert
			if ( m_IsValid ) Entries = (EventEntry[])aEntries.ToArray(typeof(EventEntry));
		}
	}
}
