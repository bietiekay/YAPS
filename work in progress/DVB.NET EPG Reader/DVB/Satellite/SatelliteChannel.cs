...
namespace JMS.DVB.Satellite
{
    ...
    public class SatelliteChannel : Channel
	{
		...
		public static SatelliteChannel Create(string settings, int lnbIndex)
		{
			...
			bool DVBS2 = (items.Length > 4);

			// Check
			if (DVBS2 && !items[4].Trim().Equals("S2")) return null;
		...
		/// Report self.
		/// </summary>
		/// <returns>Some representation of this transponder.</returns>
		public override string ToString()
		{
			// Format our data
			return string.Format("{0},{1},{2},{3},DVB-S{4}", Frequency, Power, SymbolRate, Viterbi, S2Modulation ? "2" : string.Empty);
		}
	}
}
